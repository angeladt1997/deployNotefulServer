    ...
    for (const [key, value] of Object.entries(newNotes))
      if (value == null)
        return res.status(400).json({
          error: { message: `You ain't got '${key}' in request body` },
        });
...
notesRouter
      ...
      .then((notes) => {
        if (!notes) {
          return res.status(404).json({
            error: { message: `Where did you get that note from? because you didn't put it here` },
          });
        }
        res.notes = notes;
        next();
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    if (numberOfValues === 0)
      return res.status(400).json({
        error: {
          message: `'Imma need a name or something, please'`,
        },
      });
